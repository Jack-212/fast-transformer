...
class TransformersBaseTokenizer(BaseTokenizer):
    ...
    def __init__(self, model_name, pt_tokenizer):
        self.model_name = model_name
        self.pt_tokenizer = pt_tokenizer
        self.max_seq_len = pt_tokenizer.max_len
    ...
    def tokenizer(self, t:str) -> List[str]:
        ...
        cls_token = self.pt_tokenizer.cls_token # classification token (bos)

        sep_token = self.pt_tokenizer.sep_token # seperation token
        ...
        if self.model_name in list(ROBERTA_PRETRAINED_CONFIG_ARCHIVE_MAP):
            ...
            tokens = self.pt_tokenizer.tokenize(t, add_prefix_space=True)[:self.max_seq_len - 2]
            
        else:
            tokens = self.pt_tokenizer.tokenize(t)[:self.max_seq_len - 2]
